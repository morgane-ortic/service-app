...
- - In the main/home page itself we will have a `widget that lets the user to choose the kind of service they want`, it will be `on the left half of the screen`.   It will be a list of items consisting of different massage types such as 'Full body massage', 'Shoulder Massage', 'Leg Massage'.   We can show the list on the screen by making a widget in the home page that is scrollable enabling the user to press on an option.   Or a drop-down menu with a list of massage types that can be selected.
